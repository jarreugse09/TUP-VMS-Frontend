      ...
      <div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
              <Text strong style={{ fontSize: 15, color: "#141414", lineHeight: 1.3 }}>
                {selectedRequest.userId?.firstName} {selectedRequest.userId?.surname}
              </Text>
              <span style={{
            ...
            <Text type="secondary" style={{ fontSize: 12 }}>
              Requested on {dayjs(selectedRequest.createdAt).format("MMM DD, YYYY • hh:mm A")}
            </Text>
          </div>
          ...
          <div style={{
            ...
            <Text style={{ fontSize: 12, fontWeight: 600, color: statusColors[status] || "#6b7280" }}>
              {status}
            </Text>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ padding: "16px 20px", borderRight: "1px solid #f0f0f0" }}>
            <Text type="secondary" style={{ fontSize: 10, letterSpacing: "0.5px", textTransform: "uppercase", display: "block", marginBottom: 8 }}>
              Current State
            </Text>
            {isPhotoReq ? (
               <Avatar shape="square" size={64} src={selectedRequest.userId?.photoURL} style={{ borderRadius: 8, border: "1px solid #d9d9d9" }} />
            ) : (
              <Text strong style={{ fontSize: 16, color: "#595959" }}>{selectedRequest.userId?.qrString || "No QR Set"}</Text>
            )}
          </div>
          <div style={{ padding: "16px 20px" }}>
            ...
            {isPhotoReq ? (
              <a href={toAssetUrl(selectedRequest.newPhotoImage)} target="_blank" rel="noreferrer">
                <div style={{ position: 'relative', width: 64, height: 64 }}>
                   <Avatar shape="square" size={64} src={toAssetUrl(selectedRequest.newPhotoImage)} style={{ borderRadius: 8, border: "2px solid #ff4d4f" }} />
                   <div style={{ position: 'absolute', top: -5, right: -5, background: '#ff4d4f', borderRadius: '50%', width: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <Text strong style={{ fontSize: 16, color: "#ff4d4f" }}>
                {selectedRequest.newQRString || selectedRequest.newQR || "N/A"}
              </Text>
            )}
          </div>
        </div>
        ...
        <div style={{ padding: "20px 24px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {/* Request Type Item */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px", background: "#fff", border: "1px solid #f0f0f0", borderRadius: 8 }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "#e6f7ff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <EnvironmentOutlined style={{ color: "#1890ff" }} />
              </div>
              <div>
                <Text type="secondary" style={{ fontSize: 11, display: "block" }}>Request Type</Text>
                <Text strong>{isPhotoReq ? "Profile Photo Update" : "QR Code Identification"}</Text>
              </div>
            </div>

            {/* Reason Item */}
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px", background: "#fff7e6", border: "1px solid #ffd591", borderRadius: 8 }}>
              ...
              <div>
                <Text type="secondary" style={{ fontSize: 11, display: "block" }}>Reason for Request</Text>
                <Text style={{ color: "#874d00" }}>{selectedRequest.reason || "No reason provided."}</Text>
              </div>
            </div>
            ...
            {!isPhotoReq && selectedRequest.newQRImage && (
               <div style={{ textAlign: 'center', marginTop: 8 }}>
                 <a href={toAssetUrl(selectedRequest.newQRImage)} target="_blank" rel="noreferrer" style={{ fontSize: 13, fontWeight: 500, color: '#ff4d4f', textDecoration: 'underline' }}>
