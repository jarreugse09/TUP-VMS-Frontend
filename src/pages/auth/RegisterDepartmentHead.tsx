...
interface DepartmentOption { _id: string; name: string; collegeId: string; }
...
const RegisterDepartmentHead: React.FC = () => {
  ...
  const onFinish = async (rawValues: unknown) => {
    ...
    setError(null);
    try {
      await api.post('/auth/register/department-head', {
    ...
    <RegisterContainer>
      <GlassCard>
        ...
        <StyledForm layout="vertical" form={form} onFinish={onFinish}>
          ...
          {success ? (
            <Button type="primary" block onClick={() => navigate('/auth/department-head/login')}>
              Go to Login
            </Button>
          ) : !isAdmin ? (
            <Button type="primary" block onClick={() => navigate('/auth/department-head/login')}>
            ...
            <>
              ...
              <FooterLink>
                <a onClick={() => navigate('/auth/department-head/login')}>← Back to Login</a>
              </FooterLink>
            </>
          )}
        </StyledForm>
